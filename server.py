...
class ApiApp(object):
    ...
    def rx_broadcast(self):
        ...
        try:
            float(sender_created_at)
        except:
            error = 0
        
        try:
            record = recieved_json['loginserver_record'].split(",")  
            pubkey = record[1].encode(encoder=nacl.encoding.HexEncoder)
            verify_key = nacl.signing.VeriftKey(pubkey,encoder=nacl.encoding.HexEncoder)
            verify_key.verify(recieved_json['message'])
            forged = recieved_json['message'][:-1]+ bytes(int(signed[-1])^1)
...
class MainApp(object):
    ...
    def post_message(self, message='0',key='0'):
        ...
        raise cherrypy.HTTPRedirect('/posts?status='+cherrypy.session['status'])
    ...
    def privatedata_password(self, bad_attempt = 0):
        ...
        return tmpl.render(error=attempt,user_status=cherrypy.session['status'], user=cherrypy.session['username'])
...
def encrypt_private_data(box):
    ...
    try:
        blocked_pubkey= cherrypy.session['blocked_pubkeys']
        blocked_usernames = cherrypy.session['blocked_usernames']
        blocked_message_signatures = cherrypy.session['blocked_broadcast']
        blocked_words = cherrypy.session['blocked_words']
        favourite_message_signatures = cherrypy.session['favourite_message_signatures']
        friends_usernames = cherrypy.session['friends_usernames']
    except:
        blocked_pubkey= []
        blocked_usernames = []
...
def read_private_messages():
    ...
    posts = [[], [], [], [], [], [], []]
    posts_blocking = [[], [], [], [], [], [], []]
    c.execute('SELECT * FROM private_messages WHERE target_pubkey=? ORDER BY time_created DESC LIMIT 100', [cherrypy.session['pub_key'], ])
    data = c.fetchall()
